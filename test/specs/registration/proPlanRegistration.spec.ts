...
test.describe('Admin registration @register', () => {
    test('Should register admin user with correct credentials for professional plan', async ({
        loginPage,
        registerPage,
    }) => {
        ...
        await registerPage.clickProfessionalPlan();

        //Assertion to confirm that plan is displayed on the registration screen
        await expect(registerPage.professionalPlanCardTitle).toContainText('Professional Plan');
        ...
        await expect(registerPage.planName).toContainText('Professional Plan');
